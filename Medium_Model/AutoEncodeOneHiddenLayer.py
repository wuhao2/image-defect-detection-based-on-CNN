...
#网络模型参数
"""
n_hidden_units = 256#隐藏层的神经元个数，让编码器和解码器都有相同规模的隐藏层，可以改为128，即784--->128压缩的更厉害,编码效率差些，看效果
n_input_units = 784
n_output_units = n_input_units#解码器输出层神经元个数必须等于输入层数据的units数量
"""
#定义一个张量汇总函数，用于Tensorflow的可视化（均值，标准差，最大最小值，直方图）
"""
...
with tf.Graph().as_default():
    with tf.name_scope('ForwardPrediction'):
        #计算图输入
        with tf.name_scope('X_Origin'):
            X_Origin = tf.placeholder(tf.float32, [None, n_input_units])
        #构建遍码器模型
        with tf.name_scope('Encoder'):
            X_code = Encoder(X_Origin, tf.nn.sigmoid)#可以改为softplus, tf.nn模块下面有很多的模块
        #构建解码器模型
    ...
    with tf.name_scope('BackProgation'):
        #定义优化器，训练节点
        with tf.name_scope('Train'):#反向求导计算梯度，应用优化器更新每个节点的weight和biase
            Optimizer = tf.train.RMSPropOptimizer(learning_rate)#RMS优化器
            Train = Optimizer.minimize(Loss)
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~BackProgation finished~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    print("~~~~~~~~~~~~~~~~write into calculation graph， show on tensorboard webUI~~~~~~~~~~~~~~~~")#虽然没有启动会话，但是还是可以生成计算图
    summary_write = tf.summary.FileWriter(logdir='logs', graph=tf.get_default_graph())
    summary_write.flush()

    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~Run the tensorflow calculation graph~~~~~~~~~~~~~~~~~~~~~~~~")
    # 加载数据集
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            if epoch % display_step == 0:
                print("Epoch:", '%04d' %(epoch+1), "loss:", '{:.9f}'.format(loss))#输出loss
        ...
        print("~~~~~~~~~~~~~~~~~~~~~~testing and output 4 picture completely~~~~~~~~~~~~~~~~~~~~~~~~")
