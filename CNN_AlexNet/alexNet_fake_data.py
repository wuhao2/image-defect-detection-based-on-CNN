...
csv_dir = './logs/dataResult/alexNet.csv'
#数据集中图像参数
image_size = 224
image_channel = 3
n_classes = 1000
...
def Inference(images_holder):
    ...
    with tf.name_scope('Conv2d_1'):
        weights = WeightVariable(shape=[11,11,image_channel,conv1_kernels_num],
...
######################################################################################################
    ...
    with tf.name_scope('GetTrainBatch'):#训练样本需要扩充
        ...
        tf.summary.image('images' ,images_train, max_outputs=8)##获取样本训练图像
    #定义获取测试样本批次的节点
    with tf.name_scope('GetTestBatch'):#测试样本不需要扩充
        images_test, labels_test = get_faked_test_batch(batch_size=batch_size )
        tf.summary.image('images' ,images_train, max_outputs=8)#获取样本测试图像
    ...
    with tf.Session() as sess:
        ...
        print("Per batch size：", batch_size)#32
        print("Train sample count per epoch:", num_examples_per_epoch_for_train)#1000
        print("total batch count per epoch:", total_batches)#1000/32
        ...
        results_file = open(csv_dir, 'w', newline='')
