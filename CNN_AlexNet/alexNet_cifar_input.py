...
def read_cifar10(filename_queue):
    ...
    result.label = tf.cast(
        tf.strided_slice(record_bytes, [0], [label_bytes]), tf.int32)  #label_bytes=1，切片得到第0个位置即label（uint8）----tf.int32
...
def _generate_image_and_label_batch(image, label, min_queue_examples,
    ...
    print("generate cifar batch success。。。。。")
...
def distorted_inputs(cifar10or20or100, data_dir, batch_size):
    ...
    if cifar10or20or100 == 100 :
        filenames = [os.path.join(data_dir, 'train.bin')]#返回../cifar100_dataset/cifar-100-binary/train.bin
        read_cifar = read_cifar100
...
def inputs(cifar10or20or100, eval_data, data_dir, batch_size):
    ...
    print ('Filling queue with %d CIFAR images before starting to test. '
           'This will take a few minutes.' % min_queue_examples)#产生测试样本队列
