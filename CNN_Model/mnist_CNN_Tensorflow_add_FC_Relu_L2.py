...
with tf.Graph().as_default():
    ...
    with tf.name_scope('Inference'):
        ...
        with tf.name_scope('FeatsReshape'):
            features = tf.reshape(pool_out, [-1, 12*12*conv1_kernels_num])#-1表示样本个数未知

        #非线性全连接层,此处的参数是最多的
        with tf.name_scope('Fc_Relu'):
            #不断的改变这个参数，检测网络的性能
            #实验表明：fcl_units_num越大，过拟合情况越严重
            fcl_units_num = 100
            weights = WeightVariable(shape=[12*12*conv1_kernels_num, fcl_units_num], name_str='weights')
            biases = BiasesVariable(shape=[fcl_units_num], name_str='biases')
            fc_out = FullyConnection(features, weights, biases,
                                            activate=tf.nn.relu, #恒等映射，此函数不在tf.nn下面
                                            act_name='relu')
            #为noliner全连接层添加 L2正则化损失
            with tf.name_scope('L2_Loss'):
                weights_loss = tf.nn.l2_loss(weights)


        #线性全连接层,最后一层，此处的参数是最多的
        with tf.name_scope('FullyConnection'):
            weights = WeightVariable(shape=[fcl_units_num, n_classes], name_str='weights')
            biases = BiasesVariable(shape=[n_classes], name_str='biases')
            Y_pred_logits = FullyConnection(fc_out, weights, biases,
                                            activate=tf.identity, #恒等映射，此函数不在tf.nn下面
                                            act_name='logits')

    #定义损失层
    with tf.name_scope('Loss'):
        wd = 0.00001 #正则化系数,通过做实验：不断得改变这个值，进行性能优化
        #交叉熵损失
        ...
        with tf.name_scope('TotalLoss'):
            ...
            #总体损失 = 经验损失 + 正则化损失
            total_loss = corss_entropy_loss + weights_loss
    ...
    with tf.name_scope('Train'):
        ...
        trainer = optimizer.minimize(total_loss, global_step=global_step)#计算梯度和应用梯度
        ...
        for epoch in range(training_epochs):
            ...
            for batch_idx in range(total_batches):
                ...
                if training_step % display_step == 0: #当batch_idx==10,第一次进入if，进行评估，依次进入20，30，40....
                    ...
                    #计算当前模型在验证集上的损失和分类准确率, 验证集上的样本参数是没有被更新过的，所以Loss大些，准确率也低一些
                    validation_loss, validation_acc = EvaluateMode10nDataset(sess,
                                                                   mnist.validation.images,
                                                                   mnist.validation.labels,)

                    #计算模型得正则化损失
                    l2_loss = sess.run(weights_loss)
