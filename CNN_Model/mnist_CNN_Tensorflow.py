...
def EvaluateMode10nDataset(sess, images, labels):
    ...
    if (n_samples <= per_batch_size):
        ...
        for idx in range(batch_count):
            batch_loss, batch_acc = sess.run([corss_entropy_loss, accuracy],
...
with tf.Graph().as_default():
    ...
    with tf.name_scope('Inference'):
        ...
        with tf.name_scope('Conv2d'):
            weights = WeightVariable(shape=[5, 5, 1, 16], name_str='weights')
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            for batch_idx in range(total_batches):
                ...
                if training_step % display_step == 0:  # 当batch_idx==10,第一次进入if，进行评估，依次进入20，30，40....
                    ...
                    # 如果不进行切片，可能导致训练集上的准确率和损失比验证集大都大，因为评估了没有经过训练的样本
        ...
        # 将评估结果保存到文件
        result_file = open('evaluate_results.csv', 'w', newline='')
