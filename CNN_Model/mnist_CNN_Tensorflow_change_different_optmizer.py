...
with tf.Graph().as_default():
    ...
    with tf.name_scope('Train'):
        ...
        #不可训练的参数，模型被训练一遍（模型参数可训练参数被更新一次），则glbal_step 自增1
        ...
        trainer = optimizer.minimize(corss_entropy_loss, global_step=global_step)
        #计算梯度和应用梯度, 每次调用minimize， global_step加1
    ...
    with tf.Session() as sess:
        ...
        #training_step做的工作类似global_step
        ...
        for epoch in range(training_epochs):
            ...
            for batch_idx in range(total_batches):
                ...
                # training_step += 1
                training_step = sess.run(global_step)#同理
